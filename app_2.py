...
async def upload_file(file: UploadFile = File(...)):
    ...
    # Escape the file content for JavaScript
    file_content_json = json.dumps(file_content)  # Convert to JSON string
    ...
    <body>
        ...
        <button onclick="augment()">Show Augmentation</button>  <!-- Call augment without arguments -->
        <button onclick="reset()">Reset</button>  <!-- Reset button -->
        <div id="tokenization-result"></div>
        <div id="augmentation-result"></div>
        
        <script>
            let fileContent = {file_content_json};  // Store file content in a global variable
            ...
            async function augment() {{  // No parameters needed
                const response = await fetch('/augment', {{
                    method: 'POST',
                    headers: {{
                        'Content-Type': 'application/json'
                    }},
                    body: JSON.stringify({{ content: fileContent }})  // Use the global variable
                }});
                const result = await response.json();
